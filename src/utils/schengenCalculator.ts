 ...
 */
export function isDateInStays(dateStr: string, stays: SchengenStay[]): boolean {
...
/**
 * Calculate the Schengen 90/180 status for a given reference date (default: today)
 */
export function calculateSchengen(
  ...
  let simulatedStays = [...stays];
  let futureContinuousDays = 0;
  let testDate = refDate;

  // If already in Schengen today, check how many more days continuously
  for (let i = 0; i < 95; i++) {
    const candidateDate = addDays(refDate, i);
    const candidateStr = format(candidateDate, 'yyyy-MM-dd');
    
    // Check if adding this day keeps all 180-day windows <= 90
    ...
    for (const d of daysInCandidateWindow) {
      ...
      // In simulation, if d >= refDate and d <= candidateDate, treat as Schengen
      const inSimulated = isBefore(d, refDate)
        ? isDateInStays(dStr, stays)
        : !isAfter(d, candidateDate);
