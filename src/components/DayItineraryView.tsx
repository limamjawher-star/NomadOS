...
export const DayItineraryView: React.FC<DayItineraryViewProps> = ({
  ...
  const getCategoryColor = (cat: DayItineraryActivity['category']) => {
    switch (cat) {
      ...
      case 'food':
        return 'border-orange-400 bg-orange-50/50';
      case 'coworking':
        return 'border-stone-400 bg-stone-100/50';
      case 'free':
  ...
  return (
    <div id="day-by-day-itinerary-module" className="space-y-5">
      ...
      <div className="bg-white rounded-3xl p-4 border border-stone-200/80 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-orange-100 text-orange-700 flex items-center justify-center font-bold">
            <Calendar className="w-4 h-4" />
          </div>
          <div>
            <span className="text-[10px] font-extrabold uppercase tracking-widest text-stone-400">Day Plan</span>
            <h3 className="text-base font-extrabold text-stone-900">{selectedDay}</h3>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3 py-1.5 bg-stone-100 hover:bg-stone-200 text-stone-700 font-bold rounded-xl text-xs flex items-center gap-1 transition-colors"
          >
          ...
          <button
            ...
            className="px-3.5 py-1.5 bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-700 hover:to-orange-700 text-white font-black rounded-xl text-xs flex items-center gap-1.5 shadow-md shadow-orange-500/20 transition-all transform active:scale-95"
          >
        ...
        {activities.map((act) => {
          if (act.isFreeTime) {
            return (
              ...
              >
                <div className="w-14 text-right text-xs font-bold text-stone-400">
                  {act.time}
                </div>
          ...
          return (
            <div key={act.id} className="flex items-start gap-4 relative z-10 group">
              {/* Left Time label */}
              <div className="w-14 pt-3.5 text-right text-xs font-extrabold text-stone-700 shrink-0">
                {act.time}
              </div>

              {/* Main Activity Card */}
              <div 
                className={`flex-1 bg-white rounded-2xl p-4 border border-stone-200/80 shadow-sm border-l-4 ${getCategoryColor(act.category)} transition-all hover:border-orange-300 hover:shadow-md`}
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    {act.locationName && (
                      <div className="w-7 h-7 rounded-lg bg-stone-100 hover:bg-orange-50 text-stone-500 hover:text-orange-600 flex items-center justify-center transition-colors">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-stone-200 space-y-5 animate-in fade-in zoom-in-95">
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 bg-orange-50 rounded-2xl border border-orange-100 text-xs text-stone-900 flex items-start gap-2">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                className="flex-1 py-2.5 bg-orange-600 hover:bg-orange-700 text-white font-extrabold rounded-xl text-xs shadow-md shadow-orange-600/30 flex items-center justify-center gap-1.5"
              >
                {isGenerating ? (
                  <>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-stone-200 space-y-4 animate-in fade-in zoom-in-95">
            ...
            <div className="bg-stone-50 rounded-2xl p-4 border border-stone-200/80 space-y-2 text-center">
              <div className="w-12 h-12 rounded-2xl bg-orange-600 text-white mx-auto flex items-center justify-center shadow-lg shadow-orange-500/25">
            ...
            <button
              onClick={() => setSelectedAttachment(null)}
              className="w-full py-2.5 bg-orange-600 hover:bg-orange-700 text-white font-extrabold rounded-xl text-xs transition-colors"
            >
              Done
            </button>
          </div>
