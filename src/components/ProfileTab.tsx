  ...
  return (
    <div id="profile-view" className="space-y-6 pb-28 max-w-2xl mx-auto px-4 pt-4">
        ...
        <div className="flex items-center gap-2">
          {!state.user.isPro ? (
            <button
              ...
              className="px-3.5 py-1.5 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white text-xs font-black rounded-full shadow-md shadow-indigo-600/20 flex items-center gap-1.5 transition-all hover:from-indigo-700 hover:to-indigo-800"
            >
              <Crown className="w-3.5 h-3.5 text-amber-300" /> <span>Upgrade Pro</span>
            </button>
          ) : (
      ...
      <div className="bg-white rounded-3xl p-6 border border-slate-200/90 shadow-sm flex flex-col items-center text-center space-y-4">
        <div className="relative">
          <img
            ...
            alt={state.user.name}
            className="w-24 h-24 rounded-full object-cover border-4 border-indigo-500/20 shadow-md ring-4 ring-indigo-500/10"
          />
          {state.user.isPro && (
            <div className="absolute -bottom-1 -right-1 bg-gradient-to-tr from-amber-500 to-indigo-600 text-white p-1 rounded-full shadow-md text-xs">
              👑
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <p className="text-xs font-bold text-indigo-600 mt-0.5">{state.user.tag}</p>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          <span className="flex items-center gap-1 text-xs font-bold bg-slate-100 text-slate-700 px-3 py-1 rounded-full border border-slate-200/80">
            <MapPin className="w-3.5 h-3.5 text-indigo-500" /> {state.currentCity}
          </span>
          <span className="flex items-center gap-1 text-xs font-bold bg-slate-100 text-slate-700 px-3 py-1 rounded-full border border-slate-200/80">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-sm text-center">
          <Plane className="w-4 h-4 text-indigo-600 mx-auto mb-1" />
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-sm text-center">
          <Heart className="w-4 h-4 text-indigo-600 mx-auto mb-1" />
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2.5">
          {state.user.countriesVisited.map((code) => {
            ...
            return (
              <div
                key={code}
                className="bg-white rounded-2xl p-3 border border-slate-200/90 shadow-sm flex flex-col items-center text-center space-y-1"
              >
          ...
          <button
            onClick={() => setIsAddCountryOpen(true)}
            className="rounded-2xl p-3 border-2 border-dashed border-slate-200 hover:border-indigo-400 hover:bg-indigo-50/20 text-slate-400 hover:text-indigo-600 transition-all flex flex-col items-center justify-center space-y-1 min-h-[86px]"
          >
          ...
          <div className="w-full max-w-md bg-white rounded-3xl p-6 border border-slate-200 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSaveProfile} className="space-y-3">
              ...
              <div className="pt-2 flex gap-2">
                <button
                  type="submit"
                  className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs shadow-md shadow-indigo-600/20"
                >
                  Save Changes
                </button>
                <button
