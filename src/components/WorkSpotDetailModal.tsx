...
export const WorkSpotDetailModal: React.FC<WorkSpotDetailModalProps> = ({
  ...
  return (
    <div 
      ...
      className="fixed inset-0 z-50 bg-black/75 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 overflow-y-auto"
      onClick={(e) => {
    ...
    >
      <div 
        ...
        className="bg-white rounded-3xl w-full max-w-3xl max-h-[92vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200 my-auto"
      >
        ...
        <div className="relative h-64 sm:h-72 w-full bg-slate-900 shrink-0 overflow-hidden group">
          ...
          <div className="absolute top-4 left-4 right-4 flex items-center justify-between z-10">
            <div className="flex items-center gap-2">
              <span className="px-3 py-1 rounded-full bg-orange-500/95 backdrop-blur-md text-white text-xs font-black uppercase tracking-wider flex items-center gap-1.5 shadow-md">
                {getCategoryIcon()}
                <span>{getCategoryLabel()}</span>
              </span>
              <span className="px-2.5 py-1 rounded-full bg-black/60 backdrop-blur-md text-white/90 text-xs font-bold border border-white/20">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={handleShare}
                className="w-9 h-9 rounded-full bg-black/50 text-white flex items-center justify-center hover:bg-black/70 backdrop-blur-md transition-all shadow-md"
                title="Share spot"
              >
              ...
              <button
                type="button"
                onClick={onClose}
                className="w-9 h-9 rounded-full bg-black/60 text-white flex items-center justify-center hover:bg-black/90 backdrop-blur-md transition-all shadow-md"
                title="Close"
              >
                <X className="w-5 h-5" />
              </button>
          ...
          <div className="absolute bottom-4 left-4 right-4 text-white z-10">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <span className="px-2 py-0.5 rounded-md bg-emerald-500 text-white text-[10px] font-black uppercase tracking-wider shadow-sm flex items-center gap-1">
            ...
            <h2 className="text-2xl sm:text-3xl font-black font-display drop-shadow-md text-white leading-tight">
              {spot.name}
            </h2>
        ...
        <div className="px-5 py-3 bg-slate-50 border-b border-slate-200/80 flex items-center justify-between gap-2 overflow-x-auto no-scrollbar">
          <div className="flex items-center gap-2 shrink-0">
            <button
              type="button"
              onClick={handleDirections}
              className="px-3.5 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-sm shadow-orange-500/20 transition-all active:scale-95 cursor-pointer"
            >
            ...
            <button
              ...
              className="px-3 py-2 bg-white hover:bg-slate-100 text-slate-800 border border-slate-200 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors shadow-2xs cursor-pointer"
            >
              <CalendarPlus className="w-3.5 h-3.5 text-orange-600" />
          ...
          <button
            ...
            className="text-xs font-bold text-slate-600 hover:text-slate-900 flex items-center gap-1 shrink-0 px-2.5 py-1.5 rounded-lg hover:bg-slate-200/60 transition-colors"
        ...
        {/* Tab Navigation */}
        <div className="px-5 pt-3 border-b border-slate-200 flex items-center gap-4 text-xs font-bold bg-white">
          <button
            type="button"
            ...
            <div className="space-y-6 animate-in fade-in duration-150">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1.5">
                    <span className="text-[10px] font-extrabold uppercase text-slate-500">Coffee Pricing</span>
                  ...
                  <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1.5">
                    <span className="text-[10px] font-extrabold uppercase text-slate-500">Brewing Methods</span>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2 flex-wrap pt-1 text-[11px] text-slate-600">
                  <span className="font-bold">Dietary Options:</span>
          ...
          {activeTab === 'specs' && (
            <div className="space-y-5 animate-in fade-in duration-150">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="p-4 rounded-2xl bg-orange-50 border border-orange-200/80 space-y-1">
                  <div className="flex items-center gap-1.5 text-orange-600 mb-1">
                    <Wifi className="w-4 h-4" />
                    <span className="text-[10px] font-black uppercase">Tested Speed</span>
                  </div>
                  <p className="text-2xl font-black text-slate-900">{spot.wifiSpeedMbps} Mbps</p>
                ...
                <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200/80 space-y-1">
                  ...
                  <p className="text-base font-black text-slate-900 leading-tight">{spot.powerOutlets}</p>
                  <p className="text-xs text-slate-600 font-semibold">
                    {spot.hasBackupPower ? '⚡ Backup Generator Ready' : 'Standard Grid Power'}
                  </p>
                  <p className="text-[11px] text-slate-400">Plugs along walls & work benches</p>
                </div>

                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
                  <div className="flex items-center gap-1.5 text-slate-700 mb-1">
                    <Volume2 className="w-4 h-4" />
                    <span className="text-[10px] font-black uppercase">Acoustics</span>
                  </div>
                  <p className="text-base font-black text-slate-900 leading-tight">{spot.noiseLevel}</p>
                  <p className="text-xs text-slate-600 font-semibold">
                    {spot.noiseLevel.includes('Zoom') || spot.noiseLevel.includes('Call')
                      ? 'Ideal for client video calls'
                      : 'Great for deep coding & focus'}
                  </p>
                  <p className="text-[11px] text-slate-400">Soft background cafe music</p>
                </div>
              </div>

              {/* Detailed Specs List */}
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200/80 divide-y divide-slate-200/70">
                <div className="py-2.5 flex items-center justify-between text-xs">
                  <span className="font-bold text-slate-500">Seating Ergonomics</span>
                  <span className="font-black text-slate-900">{spot.seatingErgonomics}</span>
                </div>
                <div className="py-2.5 flex items-center justify-between text-xs">
                  <span className="font-bold text-slate-500">Air Conditioning</span>
                  <span className={`font-black ${spot.airConditioning ? 'text-emerald-700' : 'text-slate-600'}`}>
                    {spot.airConditioning ? '✓ Strong Indoor A/C + Fans' : 'Open-Air Tropical Breeze'}
                  </span>
                </div>
                <div className="py-2.5 flex items-center justify-between text-xs">
                  <span className="font-bold text-slate-500">Day Pass Price</span>
                  <span className="font-black text-slate-900">
                    {spot.dayPassUSD ? `$${spot.dayPassUSD} / day` : 'Free with food/coffee purchase'}
                  </span>
                </div>
                <div className="py-2.5 flex items-center justify-between text-xs">
                  <span className="font-bold text-slate-500">Laptop Stay Policy</span>
                  <span className="font-black text-slate-900">
              ...
              <div className="space-y-2">
                <h5 className="text-xs font-black text-slate-900">Nomad Features & Tags</h5>
                <div className="flex flex-wrap gap-1.5">
                  {spot.tags.map((tag, idx) => (
                    <span
                      key={idx}
                      className="px-2.5 py-1 rounded-lg bg-orange-50 border border-orange-200 text-orange-800 text-xs font-bold"
                    >
                      #{tag}
                    </span>
                  ))}
            ...
            <div className="space-y-5 animate-in fade-in duration-150">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-3">
                ...
                <div className="p-3 bg-white rounded-xl border border-slate-200 flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-600">Daily Schedule</span>
                  <span className="text-xs font-black text-slate-900">{spot.openingHours}</span>
                </div>

                <div className="text-xs text-slate-500 space-y-1">
                  <p><strong>💡 Nomad Tip:</strong> Arrive before 10:00 AM for the best plug-equipped tables and morning quiet focus. Late afternoons (4 PM - 7 PM) transition into vibrant sunset social hours.</p>
                </div>
              </div>

              {/* Amenities Grid */}
              <div className="space-y-2.5">
                <h5 className="text-xs font-black text-slate-900 font-display">Facilities & House Rules</h5>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center gap-2 text-xs font-bold text-slate-700">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
              ...
              {/* Contact & Socials */}
              <div className="space-y-2.5">
                <h5 className="text-xs font-black text-slate-900 font-display">Location & Social Channels</h5>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                  <button
                    type="button"
                    onClick={handleDirections}
                    className="p-3 rounded-xl bg-slate-50 hover:bg-orange-50 border border-slate-200/80 text-left flex items-center justify-between transition-colors group cursor-pointer"
                  >
                    <div className="flex items-center gap-2.5">
                      <MapPin className="w-4 h-4 text-orange-500" />
                      <span className="font-bold text-slate-800 group-hover:text-orange-600">Open in Google Maps</span>
                    </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-slate-400 font-bold">Community Tag</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-5 animate-in fade-in duration-150">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-14 h-14 rounded-2xl bg-amber-500 text-white flex flex-col items-center justify-center font-black shadow-sm">
                    <span className="text-xl leading-none">{spot.rating}</span>
                    <span className="text-[10px] font-bold text-amber-100">/ 5.0</span>
                  </div>
                  <div>
                    <h5 className="text-sm font-black text-slate-900 font-display">Verified Nomad Rating</h5>
                    <p className="text-xs text-slate-500">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl text-xs flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
                >
              ...
              {/* Review Form */}
              {showReviewForm && (
                <form onSubmit={handleSubmitReview} className="p-4 rounded-2xl bg-orange-50/70 border border-orange-200 space-y-3 animate-in fade-in">
                  <h5 className="text-xs font-black text-slate-900 font-display">Log Your Speed Test & Experience</h5>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Overall Rating</label>
                      <select
                        value={rating}
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Wi-Fi Tested (Mbps)</label>
                      <input
                        type="number"
                        min={5}
                        max={1000}
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Noise Atmosphere</label>
                      ...
                      {(['Quiet Focus', 'Moderate / Cafe Ambience', 'Lively', 'Zoom Friendly'] as const).map((lvl) => (
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-colors ${
                            noise === lvl ? 'bg-orange-500 text-white' : 'bg-white border border-slate-300 text-slate-700'
                          }`}
                        >
                          {lvl}
                        </button>
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Nomad Notes & Coffee Review</label>
                    <textarea
                      rows={2}
                      value={comment}
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs rounded-xl flex items-center gap-1.5 shadow-sm"
                  >
              ...
              <div className="space-y-3">
                {localReviews.map((rev) => (
                  <div key={rev.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-2">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5">
                            <h6 className="font-extrabold text-xs text-slate-900 font-display">{rev.author}</h6>
                            {rev.verifiedNomad && (
                              <span className="flex items-center gap-0.5 px-1.5 py-0.2 rounded bg-orange-100 text-orange-700 text-[9px] font-bold">
                    ...
                    <div className="flex items-center gap-3 text-[10px] font-bold text-slate-400 pt-1 border-t border-slate-200/60">
