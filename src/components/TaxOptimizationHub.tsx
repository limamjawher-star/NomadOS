...
export const TaxOptimizationHub: React.FC<TaxOptimizationHubProps> = ({ state, onOpenPricing }) => {
  ...
  // Current Country 183-day residency alert
  const currentCityParts = (state.currentCity || 'Canggu, Bali').split(',');
  const currentCountry = currentCityParts[1]?.trim() || 'Indonesia';
  const daysInCurrentCountry = 68; // Simulated accurate tracker from trip logs

  return (
    <div className="space-y-4 animate-in fade-in">
      {/* Visual Header Banner */}
      <div className="relative rounded-3xl overflow-hidden border border-slate-200/90 shadow-sm bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white p-5 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-full bg-orange-500/20 text-orange-400 text-[10px] font-bold border border-orange-500/30 uppercase tracking-wider">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3.5 border border-white/15 shrink-0 text-center sm:text-right">
        ...
        <div className="bg-white rounded-3xl p-5 border border-slate-200/90 shadow-sm space-y-4">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-600 font-medium">Physical Days Outside Home Country</span>
              <span className="font-bold text-slate-900">{feieDaysAbroad} / 330 days</span>
            </div>
          ...
          <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200/80 space-y-2.5">
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-600">Simulate Days Abroad in 365d Window:</span>
              <span className="font-bold text-orange-600">{feieDaysAbroad} days</span>
            </div>
            <input
              type="range"
              min="180"
            ...
            <div className="flex items-center justify-between pt-1 text-xs">
              <span className="text-slate-600">Estimated Federal Tax Excluded:</span>
        ...
        <div className="bg-white rounded-3xl p-5 border border-slate-200/90 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <span className="text-xs text-slate-400 block font-medium">Logged Deductible</span>
          ...
          <div className="p-3.5 bg-emerald-50/70 rounded-2xl border border-emerald-200/60 flex items-center justify-between">
            <div>
          ...
          <div className="space-y-2">
            <div className="text-xs font-semibold text-slate-700">Qualifying Logged Business Expenses:</div>
            {deductibleExpenses.length > 0 ? (
              <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                {deductibleExpenses.map((exp) => (
                  <div 
                    key={exp.id}
                    className="p-2 bg-slate-50 rounded-xl border border-slate-200/60 flex items-center justify-between text-xs"
                  >
                    <div className="flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-emerald-500" />
                      <span className="font-medium text-slate-800">{exp.description}</span>
                    </div>
                    <span className="font-semibold text-slate-900">${exp.amountUSD} USD</span>
                  </div>
                ))}
              </div>
            ) : (
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-[10px] font-semibold border border-emerald-200">
                Safe (Day {daysInCurrentCountry} of 183)
              </span>
            </div>
            <p className="text-xs text-slate-500 mt-0.5">
              Spending over 183 days in {currentCountry} within 12 months may trigger unintended local tax residency on global income. You have {183 - daysInCurrentCountry} safe days remaining.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-200/90 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] text-slate-400">Updated 2026</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {GLOBAL_TAX_REGIMES.map((regime) => (
            <div
              key={regime.country}
              className="p-4 rounded-2xl border border-slate-200/80 bg-slate-50/70 hover:bg-white hover:border-orange-300 hover:shadow-sm transition-all space-y-2.5 flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <CountryFlag code={regime.flag} className="w-5 h-3.5 rounded shadow-xs" />
                    <span className="font-bold text-xs text-slate-900">{regime.country}</span>
                  </div>
                ...
                <h5 className="text-xs font-semibold text-orange-600 mt-2">
                  {regime.visaName}
                </h5>
              ...
              <div className="pt-2 border-t border-slate-200/60 flex items-center justify-between text-[10px] text-slate-400">
                ...
                <span className="font-bold text-emerald-600">{regime.savingsScore}</span>
              </div>
            </div>
          ))}
        </div>
