...
import { TaxOptimizationHub } from '../tax/TaxOptimizationHub';
...
export const FinanceTab: React.FC<FinanceTabProps> = ({
  ...
  return (
    <div id="finance-planning-view" className="space-y-5 pb-28 max-w-2xl mx-auto px-4 pt-3">
      {/* 1. SECTION TITLE & SUB-NAV TABS */}
      <div className="bg-white rounded-xl p-4 border border-stone-200/80 shadow-xs space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-stone-500 font-normal">
                Income streams, digital nomad budget & runway planner
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="px-3.5 py-2 bg-orange-500 hover:bg-orange-600 text-white text-xs font-semibold rounded-xl shadow-xs flex items-center gap-1.5 transition-colors cursor-pointer"
          >
        ...
        {/* 5 Segmented Subtabs */}
        <div className="flex overflow-x-auto gap-1.5 p-1 bg-stone-100/90 rounded-xl border border-stone-200/80 text-xs no-scrollbar">
          <button
            onClick={() => setSubView('overview')}
            className={`flex-1 min-w-[75px] py-1.5 px-3 rounded-lg transition-all whitespace-nowrap text-center cursor-pointer ${
              subView === 'overview'
          ...
          <button
            ...
            className={`flex-1 min-w-[85px] py-1.5 px-3 rounded-lg transition-all whitespace-nowrap text-center cursor-pointer ${
              subView === 'expenses'
                ? 'bg-orange-500 text-white shadow-xs font-semibold'
                : 'text-stone-500 font-normal hover:text-stone-800'
            }`}
          ...
          <button
            ...
            className={`flex-1 min-w-[80px] py-1.5 px-3 rounded-lg transition-all whitespace-nowrap text-center cursor-pointer ${
              subView === 'income'
                ? 'bg-orange-500 text-white shadow-xs font-semibold'
                : 'text-stone-500 font-normal hover:text-stone-800'
            }`}
          ...
          <button
            ...
            className={`flex-1 min-w-[85px] py-1.5 px-3 rounded-lg transition-all whitespace-nowrap flex items-center justify-center gap-1 cursor-pointer ${
              subView === 'tax'
                ? 'bg-orange-500 text-white shadow-xs font-semibold'
                : 'text-stone-500 font-normal hover:text-stone-800'
            }`}
          ...
          <button
            ...
            className={`flex-1 min-w-[75px] py-1.5 px-3 rounded-lg transition-all whitespace-nowrap flex items-center justify-center gap-1 cursor-pointer ${
              subView === 'planner'
                ? 'bg-orange-500 text-white shadow-xs font-semibold'
                : 'text-stone-500 font-normal hover:text-stone-800'
            }`}
      ...
      {subView === 'overview' && (
        <div className="space-y-5 animate-in fade-in">
          ...
          <div className="relative rounded-xl overflow-hidden border border-stone-200/80 shadow-xs bg-stone-950">
            <img 
            ...
            <div className="absolute top-3.5 left-3.5 right-3.5 flex items-center justify-between">
              <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 backdrop-blur-xs border border-emerald-400/30 text-emerald-300 text-[11px] font-medium flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                <span>Financial Freedom Score: 94/100 · Elite Buffer</span>
              </span>

              <span className="px-2.5 py-1 rounded-full bg-black/40 backdrop-blur-xs border border-white/20 text-white text-[11px] font-medium">
            ...
            <div className="absolute bottom-3.5 left-3.5 right-3.5 flex items-end justify-between">
              <div>
                <span className="text-[10px] uppercase font-semibold text-orange-300 tracking-wider block">
                  Net Monthly Free Cash Flow
                </span>
                <h3 className="text-2xl sm:text-3xl font-bold text-white font-display leading-none mt-1">
                  +${netMonthlySavingsUSD.toLocaleString()} / mo
                </h3>
                <p className="text-xs text-white/80 font-normal mt-1">
                  Saving <strong className="text-emerald-300 font-semibold">{savingsRatePercentage}%</strong> of income after {state.taxBufferPercentage}% tax reserve
                </p>
              </div>

              <button
                onClick={() => setSubView('planner')}
                className="px-3.5 py-2 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-semibold text-xs shadow-xs flex items-center gap-1 cursor-pointer transition-colors"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            ...
            <div 
              onClick={() => setSubView('income')}
              className="bg-white rounded-xl p-4 border border-stone-200/80 shadow-xs cursor-pointer hover:border-stone-300 transition-all flex flex-col justify-between"
            >
              <div className="flex items-center justify-between text-[10px] font-semibold uppercase text-stone-400 tracking-wider">
                <span>Gross Income</span>
                <ArrowUpRight className="w-3.5 h-3.5 text-emerald-500" strokeWidth={1.75} />
              </div>
              <h4 className="text-xl font-bold text-stone-900 font-display mt-1">
            ...
            >
              <div className="flex items-center justify-between text-[10px] font-semibold uppercase text-stone-400 tracking-wider">
                ...
                <CreditCard className="w-3.5 h-3.5 text-orange-500" strokeWidth={1.75} />
              </div>
              <h4 className="text-xl font-bold text-stone-900 font-display mt-1">
            ...
            >
              <div className="flex items-center justify-between text-[10px] font-semibold uppercase text-stone-400 tracking-wider">
                ...
                <PiggyBank className="w-3.5 h-3.5 text-orange-500" strokeWidth={1.75} />
              </div>
              <h4 className="text-xl font-bold text-stone-900 font-display mt-1">
          ...
          <div className="bg-white rounded-xl p-4 border border-stone-200/80 shadow-xs space-y-3">
            <div className="flex items-center justify-between">
              <div>
              ...
              <span className="px-2.5 py-1 rounded-full bg-orange-50 text-orange-700 text-xs font-medium border border-orange-200/60">
            ...
            <div className="flex items-center justify-between text-[11px] text-stone-500 font-normal">
          ...
          <div className="bg-white rounded-xl p-4 border border-stone-200/80 shadow-xs space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-semibold text-stone-900 font-display flex items-center gap-1.5">
                <Layers className="w-4 h-4 text-orange-500" strokeWidth={1.75} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
              ...
              <div 
                ...
                className="group relative rounded-xl overflow-hidden border border-stone-200 shadow-xs cursor-pointer h-28"
              >
                <img 
                ...
                <div className="absolute bottom-2.5 left-2.5 right-2.5 text-white">
                  <span className="text-[10px] font-semibold text-orange-300 uppercase tracking-wider block">Housing</span>
                  <h5 className="text-xs font-semibold leading-tight">Villas & Coliving</h5>
                  <span className="text-[11px] font-bold text-white mt-0.5 block font-display">$950/mo</span>
                </div>
              </div>
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-stone-950 via-stone-950/40 to-transparent" />
                <div className="absolute bottom-2.5 left-2.5 right-2.5 text-white">
                  <span className="text-[10px] font-semibold text-amber-300 uppercase tracking-wider block">Workspaces</span>
                  <h5 className="text-xs font-semibold leading-tight">Coworking & Cafes</h5>
                  <span className="text-[11px] font-bold text-white mt-0.5 block font-display">$185/mo</span>
                </div>
              </div>
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-stone-950 via-stone-950/40 to-transparent" />
                <div className="absolute bottom-2.5 left-2.5 right-2.5 text-white">
                  <span className="text-[10px] font-semibold text-emerald-300 uppercase tracking-wider block">Transit</span>
                  <h5 className="text-xs font-semibold leading-tight">Flights & Scooters</h5>
                  <span className="text-[11px] font-bold text-white mt-0.5 block font-display">$915/mo</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-5 border border-stone-200/80 shadow-xs space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-xl bg-orange-50 text-orange-600 flex items-center justify-center font-semibold">
              ...
              <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 text-xs font-medium border border-emerald-200">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
              {[
                ...
                <div
                  key={wallet.code}
                  className="p-3 rounded-xl bg-stone-50 border border-stone-200/80 hover:bg-orange-50/50 hover:border-orange-200 transition-colors"
                >
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-base">{wallet.flag}</span>
                    <span className="font-semibold text-stone-700 text-[11px]">{wallet.code}</span>
                  </div>
            ...
            <div className="p-3.5 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl border border-emerald-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
              ...
              <div className="shrink-0 text-right">
                <span className="text-[10px] text-emerald-700 uppercase font-medium block">Estimated Annual Savings</span>
      ...
      {subView === 'income' && (
        <div className="space-y-5 animate-in fade-in">
          ...
          <div className="relative rounded-xl overflow-hidden border border-stone-200/80 shadow-xs bg-stone-950">
            ...
            <div className="absolute bottom-3.5 left-3.5 right-3.5 flex items-end justify-between">
              ...
              <button
                ...
                className="px-3.5 py-2 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-semibold text-xs shadow-xs flex items-center gap-1.5 transition-colors cursor-pointer"
              >
                <Plus className="w-4 h-4" strokeWidth={1.75} />
          ...
          <div className="bg-white rounded-xl p-4 border border-stone-200/80 shadow-xs space-y-2.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
          ...
          <div className="space-y-2.5">
            {state.incomes.map((inc) => (
              <div
                key={inc.id}
                className="bg-white rounded-xl p-4 border border-stone-200/80 shadow-xs flex items-center justify-between hover:border-orange-200 transition-all"
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-orange-50 text-orange-600 flex items-center justify-center font-semibold text-sm shrink-0">
                    <TrendingUp className="w-5 h-5" strokeWidth={1.75} />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h5 className="text-sm font-semibold text-stone-900 font-display">{inc.source}</h5>
                      <span className="px-2 py-0.5 rounded-full bg-stone-100 text-stone-600 text-[10px] font-medium capitalize">
                        {inc.type}
                      </span>
                    </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-xl bg-stone-50 hover:bg-rose-50 text-stone-400 hover:text-rose-600 flex items-center justify-center transition-colors cursor-pointer"
          ...
          <div className="space-y-2.5">
            {filteredExpensesList.map((exp) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-orange-50 text-orange-600 flex items-center justify-center font-semibold text-sm shrink-0">
                    {exp.category === 'Accommodation' && <Building className="w-5 h-5" strokeWidth={1.75} />}
                    {exp.category === 'Flights & Transit' && <Plane className="w-5 h-5" strokeWidth={1.75} />}
                    {exp.category === 'Coworking & Cafes' && <Laptop className="w-5 h-5" strokeWidth={1.75} />}
                    {exp.category === 'Food & Groceries' && <Coffee className="w-5 h-5" strokeWidth={1.75} />}
                    {exp.category === 'Health & Visas' && <HeartPulse className="w-5 h-5" strokeWidth={1.75} />}
                    {exp.category === 'Tech & Subscriptions' && <Wifi className="w-5 h-5" strokeWidth={1.75} />}
                    {exp.category === 'Wellness & Leisure' && <Sparkles className="w-5 h-5" strokeWidth={1.75} />}
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h5 className="text-xs sm:text-sm font-semibold text-stone-900 font-display">
                        {exp.description}
                      </h5>
                      {exp.isDeductible && (
                        <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-[10px] font-medium tracking-wide border border-emerald-200/60">
                          TAX DEDUCTIBLE
                        </span>
                      )}
                    </div>
      ...
      {subView === 'planner' && (
        <div className="space-y-5 animate-in fade-in">
          ...
          <div className="relative rounded-xl overflow-hidden border border-stone-200/80 shadow-xs bg-stone-950">
            ...
            <div className="absolute top-3.5 left-3.5 right-3.5 flex items-center justify-between">
              <span className="px-2.5 py-1 rounded-full bg-orange-500/30 backdrop-blur-xs border border-orange-400/50 text-orange-200 text-[11px] font-semibold flex items-center gap-1.5">
          ...
          <div className="bg-white rounded-xl p-4 border border-stone-200/80 shadow-xs space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-base font-bold text-orange-600 font-display">
                ${customBurnUSD} / mo
              </span>
            </div>
          ...
          <div className="space-y-2.5">
            <div className="flex items-center justify-between px-1">
              ...
              <span className="text-[11px] text-stone-400 font-normal">Based on ${totalSavings.toLocaleString()} savings</span>
            </div>
              ...
              {NOMAD_RUNWAY_DESTINATIONS.map((dest) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-white rounded-xl p-3 border transition-all cursor-pointer shadow-xs hover:shadow-md ${
                      selectedCityForRunway === dest.city
                        ? 'border-orange-500 ring-2 ring-orange-500/20'
                        : 'border-stone-200/80 hover:border-orange-200'
                    }`}
                  >
                    <div className="relative rounded-lg overflow-hidden h-28 mb-3">
                      ...
                      <div className="absolute top-2 left-2 flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-black/60 backdrop-blur-xs text-white text-[10px] font-medium border border-white/20">
                      ...
                      <div className="absolute bottom-2 left-2 right-2 flex items-baseline justify-between text-white">
                        <h5 className="font-semibold text-sm">{dest.city}</h5>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center text-[10px] font-medium text-stone-600 bg-stone-50 p-2 rounded-lg">
                      <div>
                        <span className="text-stone-400 block text-[9px]">Burn/mo</span>
                        <span className="text-stone-900 font-bold font-display">${dest.monthlyBurnUSD}</span>
                      </div>
                      <div>
                        <span className="text-stone-400 block text-[9px]">Rent</span>
                        <span className="text-stone-900 font-bold font-display">${dest.rentUSD}</span>
                      </div>
                      <div>
                        <span className="text-stone-400 block text-[9px]">Coworking</span>
                        <span className="text-stone-900 font-bold font-display">${dest.coworkingUSD}</span>
                      </div>
                    </div>
                  </div>
                );
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {state.financialGoals.map((goal) => {
                ...
                return (
                  <div
                    key={goal.id}
                    className="bg-white rounded-xl p-4 border border-stone-200/80 shadow-xs space-y-3"
                  >
                    <div className="flex items-center gap-3">
                      <img
                        src={goal.imageUrl}
                      ...
                      <div className="flex-1 min-w-0">
                        <h5 className="text-xs font-semibold text-stone-900 truncate font-display">
                          {goal.title}
                        </h5>
                        <p className="text-[10px] text-stone-400 capitalize font-normal">
                    ...
                    <button
                      ...
                      className="w-full py-2 bg-orange-50 hover:bg-orange-100 text-orange-700 font-semibold rounded-xl text-xs transition-colors flex items-center justify-center gap-1 cursor-pointer"
                    >
                      <Plus className="w-3.5 h-3.5" strokeWidth={1.75} />
      ...
      {isAddExpenseOpen && (
        <div className="fixed inset-0 z-50 bg-stone-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <form 
            ...
            className="bg-white rounded-xl p-6 max-w-md w-full shadow-2xl border border-stone-200/80 space-y-4 animate-in fade-in zoom-in-95"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg bg-orange-50 text-orange-600 flex items-center justify-center border border-orange-200/60 shadow-xs">
                  <CreditCard className="w-4 h-4" strokeWidth={1.75} />
                </div>
                <h3 className="font-semibold text-stone-900 text-base font-display">
                  Log Nomad Expense
                </h3>
              </div>
              <button
                ...
                className="w-7 h-7 rounded-full bg-stone-100 text-stone-500 hover:bg-stone-200 flex items-center justify-center cursor-pointer transition-colors"
              >
            ...
            <div>
              <label className="block text-[11px] font-medium text-stone-500 mb-1.5">1-Tap Fast Fill (No Typing)</label>
              <div className="flex flex-wrap gap-1.5">
                {[
                  { label: 'Cafe Work', amount: '4.5', cat: 'Coworking & Cafes', ded: true },
                  { label: 'Cowork Pass', amount: '20', cat: 'Coworking & Cafes', ded: true },
                  { label: 'eSIM Data', amount: '14', cat: 'Tech & Subscriptions', ded: true },
                  { label: 'Nomad Meal', amount: '6.5', cat: 'Food & Groceries', ded: false },
                  { label: 'Grab Transit', amount: '5', cat: 'Flights & Transit', ded: false },
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-stone-100 hover:bg-orange-50 hover:text-orange-700 text-stone-600 text-[11px] font-medium border border-stone-200/80 transition-colors cursor-pointer"
                  >
                    {p.label} (${p.amount})
                  </button>
                ))}
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-stone-200/80 text-xs font-normal focus:outline-none focus:border-orange-500"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl border border-stone-200/80 text-xs font-medium bg-white focus:outline-none focus:border-orange-500"
                  >
              ...
              <div>
                <label className="block text-xs font-medium text-stone-700 mb-1">Notes / Location</label>
                <input
                  type="text"
                  value={expNotes}
      ...
      {isAddIncomeOpen && (
        <div className="fixed inset-0 z-50 bg-stone-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-200/60 shadow-xs">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-stone-700 mb-1">Source Name / Client</label>
                <input
                  type="text"
                  required
                  value={incSource}
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  <label className="block text-xs font-medium text-stone-700 mb-1">Monthly Amount</label>
                  <input
                    type="number"
                    required
                    min="0"
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  <label className="block text-xs font-medium text-stone-700 mb-1">Income Type</label>
                  <select
                    value={incType}
                ...
                <div>
                  <label className="block text-xs font-medium text-stone-700 mb-1">Client Country</label>
                  <input
                    type="text"
                    value={incCountry}
      ...
      {isAddGoalOpen && (
        <div className="fixed inset-0 z-50 bg-stone-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-stone-700 mb-1">Goal Title</label>
                <input
                  type="text"
                  required
                  value={goalTitle}
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  <label className="block text-xs font-medium text-stone-700 mb-1">Target Amount ($)</label>
                  <input
                    type="number"
                    required
                    min="1"
                ...
                <div>
                  <label className="block text-xs font-medium text-stone-700 mb-1">Starting Amount ($)</label>
                  <input
                    type="number"
                    min="0"
                    value={goalCurrent}
              ...
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <div>
                  <label className="block text-xs font-medium text-stone-700 mb-1">Target Date</label>
                  <input
                    type="date"
                    value={goalDeadline}
      ...
      {fundingGoalId && (
        <div className="fixed inset-0 z-50 bg-stone-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <form 
            ...
            className="bg-white rounded-xl p-6 max-w-sm w-full shadow-2xl border border-stone-200/80 space-y-4 animate-in fade-in zoom-in-95"
          >
            <div className="flex items-center justify-between">
              <h4 className="font-semibold text-stone-900 text-base font-display">Deposit to Goal</h4>
              <button
                type="button"
            ...
            <div>
              <label className="block text-xs font-medium text-stone-700 mb-1">Contribution Amount ($ USD)</label>
              <input
                type="number"
                required
                min="1"
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-stone-200/80 text-sm font-bold focus:outline-none focus:border-orange-500 font-display"
              />
            </div>
