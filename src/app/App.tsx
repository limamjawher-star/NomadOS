...
import { AIConciergeTab } from '../features/intelligence/components/ai/AIConciergeTab';
...
export function App() {
  ...
  const [activeTab, setActiveTab] = useState<'home'|'travel'|'finance'|'explore'|'social'|'me'|'ai'>('home');
  ...
  const liveTrips = useLiveQuery(() => db.trips.filter(x => !x.deleted_at).toArray(), []) || [];
  const liveExpenses = useLiveQuery(() => db.expenses.filter(x => !x.deleted_at).toArray(), []) || [];
  const liveIncomes = useLiveQuery(() => db.income.filter(x => !x.deleted_at).toArray(), []) || [];
  const liveSchengen = useLiveQuery(() => db.schengenStays.filter(x => !x.deleted_at).toArray(), []) || [];
  const liveTax = useLiveQuery(() => db.taxPresence.filter(x => !x.deleted_at).toArray(), []) || [];
  const liveGoals = useLiveQuery(() => db.goals.filter(x => !x.deleted_at).toArray(), []) || [];
      ...
      {viewMode === 'landing' ? (
        ...
        <div className="w-full min-h-screen bg-stone-50 flex flex-col">
          ...
          <main className="flex-1 w-full max-w-5xl mx-auto px-3 sm:px-4 py-4 sm:py-6 pb-24 sm:pb-28">
            ...
            {activeTab === 'ai' && <AIConciergeTab state={state as any} onNavigateTab={setActiveTab} onOpenPricing={() => setIsPricingOpen(true)} isPro={state.user?.isPro || false} />}
